...
### v1.6.0 提醒体验
- **每周多天**：“每周固定时间”可同时选择多天（如周一、三、五），并提供“工作日 / 周末 / 每天”快捷选项。
- **法定工作日**：新增循环模式“法定工作日”，跳过法定节假日、调休上班日照常提醒。内置 2025–2026 年安排（`src-tauri/data/holidays-cn.json`）；新年度安排发布后，可在数据目录（Windows 为 `%APPDATA%\TaskReminderApp\data`）放置同名 `holidays-cn.json` 覆盖，无需等待发版。没有数据的年份按周一至周五计算。
- **提醒弹窗**：一次性待办可直接点“完成”；“稍后提醒”可选默认分钟数 / 15 分钟 / 1 小时 / 明早 9:00；“全部知道了”移到右上角。
- **快速添加**：全局快捷键（默认 `Ctrl+Alt+N`）或托盘菜单打开快速添加窗口，输入内容、选提醒时间后回车保存。快捷键可在“应用设置”中录制或关闭，被其他程序占用时会提示原因。
- 注意：多设备同步时请把所有设备升级到 1.6.0，旧版本不认识“法定工作日”模式。
...
当前技术栈：**Tauri 2 + Vue 3 + TypeScript**，当前版本 **v1.6.0**。

## 功能概览
- **待办事项**：输入标题后回车即可添加，支持 Markdown 所见即所得描述；可设置一次性提醒时间，列表中按“即将到期 / 已过期”着色显示。
- **已办事项**：按标题或描述搜索，勾选即可取消完成。
- **循环提醒**：支持区间间隔、每天、法定工作日（跳过节假日、含调休）、每周（可多选）、每月固定时间与 Cron 表达式，可暂停/恢复。
- **提醒记录**：按日期与类型筛选，查看每次提醒的处理结果（已关闭 / 已推迟 / 已完成 / 待处理），支持批量删除。
- **提醒弹窗**：到点弹出，多条提醒排队展示；支持“完成”“知道了”和多档“稍后提醒”，可选提示音。
- **快速添加**：全局快捷键（默认 `Ctrl+Alt+N`）随时弹出输入框添加待办并设置提醒。
...
```
https://github.com/zhouyilong/taskReminder/releases/download/v1.6.0/TaskReminderApp_1.6.0_x64_zh-CN.msi
```
...
发布要求：
- GitHub Release 标签需使用 `v<version>` 格式，例如 `v1.6.0`
